...
当光线从表面离开时（例如反射），与辐照度相同的物理量称为辐射出射度E。用不同的物理量表示入射和出射光是有用的，因为同一点具有潜在不同的辐照度和辐射出射度。
### 14.6.3 辐射 Radiance
...
作为一个例子，在所有方向上具有相同场辐射Lf的表面上，我们可以计算辐射照度H。为了积分，我们使用经典的球坐标系，并回忆微分立体角是
...
因为我们对表面外观感兴趣，所以我们想表征表面如何反射光。 在直观层面上。对于来自 k 方向的任何入射光；在出射方向 k 附近的小立体角中存在一些散射光。我们可以通过多种方式形式化这样的概念，毫不奇怪的是标准方法 的灵感来自于构建一个简单的测量设备。 这样的装置如图 14.10 所示，其中一个小光源位于 k 方向；从表面上的一点看，探测器位于 ko 方向。 对于每个方向对 k:k 我们用探测器读取读数现在我们只需要决定如何测量光源的强度并使我们的反射函数独立于该强度。例如，如果我们用更亮的光替换光，我们就不会想要 认为表面以不同的方式反射光。我们可以在被照射的点放置一个辐射计来测量光。但是，为了获得准确的读数，
## 14.8 传递方程
## 14.9 实际中的材质
## 14.10 蒙特卡洛光线追踪